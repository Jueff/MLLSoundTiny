...
{
  MLLST_LOG(1, "MP3-TF-16P sendStack\n");
...
{
  MLLST_LOG(1, "MP3-TF-16p command: %d larg %d\n", command, argument);
...
{
  MLLST_LOG(1, "MP3-TF-16p command: %d msb: %d lsb: %d\n", command, argumentHigh, argumentLow);
...
void MultiPlayer::setModuleType(uint8_t moduleType) 
{
  if (_currentType != moduleType)
  {
    ...
    MLLST_LOG(1, "Module type changed to: %d\n", _currentType);
  }
}

void MultiPlayer::sendCmd(uint8_t command) 
{
  MLLST_LOG(1, "Module type: %d\n", _currentType);
  switch (_currentType) {
...
{
  MLLST_LOG(1, "JQ6500 cmd %d arg %d\n", command, argument);
...
{
  MLLST_LOG(1, "JQ6500 cmd %d larg %d\n", command, argument);
  command = mapJQ6500Command(command);
  _sendJQ[1] = 4;
  _sendJQ[2] = command;
